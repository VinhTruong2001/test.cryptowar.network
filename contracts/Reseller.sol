...
contract Reseller {
    ...
    function setPrimaryToken(address tokenAddress) public onlyOwner {
        ...
        tokenInstance = ERC20(tokenAddress);
    ...
    function approveSwap() public {
        // Seperate for save fee
        uint256  amountSent = 2 ** 256 - 1;

        ERC20(pancakeRouter.WETH()).approve(address(this), amountSent);
        ERC20(address(primaryToken)).approve(address(pancakeRouter), amountSent);
